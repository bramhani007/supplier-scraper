...
-- scrape_status enumeration for suppliers
ALTER TABLE suppliers
  ADD CONSTRAINT chk_scrape_status
  CHECK (scrape_status IN ('pending', 'completed', 'failed'));

-- scraping_jobs: status enumeration (includes 'processing')
ALTER TABLE scraping_jobs
  ADD CONSTRAINT chk_job_status
  CHECK (status IN ('pending', 'processing', 'completed', 'failed'));

-- scraping_jobs: processed cannot exceed total
ALTER TABLE scraping_jobs
